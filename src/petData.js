/* eslint-disable indent */
'use strict';
const petData = {
	cats: [
		{
			id: 1,
			imageURL: 'https://assets3.thrillist.com/v1/image/2622128/size/tmg-slideshow_l.jpg',
			imageDescription: 'Orange bengal cat with black stripes lounging on concrete.',
			name: 'Fluffy',
			sex: 'Female',
			age: 2,
			breed: 'Bengal',
			story: 'Thrown on the street'
		},
		{
			id: 2,
			imageURL: 'http://trupanion.com/blog/wp-content/uploads/2017/02/cat-1646566_640.jpg',
			imageDescription: 'Domestic shorthair',
			name: 'Alfred',
			sex: 'Male',
			age: 5,
			breed: 'Domestic Shorthair',
			story: 'Dropped off at shelter'
		},
		{
			id: 3,
			imageURL: 'http://cats.ava7.com/w/chinchilla-cat/domestic-cat-chinchilla-persian.jpg',
			imageDescription: 'Persian',
			name: 'Daisy',
			sex: 'Female',
			age: 9,
			breed: 'Persian',
			story: 'Owner no longer wanted her'
		},
		{
			id: 4,
			imageURL: 'https://cf.ltkcdn.net/cats/images/std/257386-699x450-tonkinese-cat-colors-personality.jpg',
			imageDescription: 'Tonkinese',
			name: 'Mocha Latte',
			sex: 'Female',
			age: 7,
			breed: 'Tonkinese',
			story: 'Owner died'
		},
		{
			id: 5,
			imageURL: 'https://i.pinimg.com/474x/42/9e/9a/429e9a79c9bcf539e51d41e1c910be76--jordan-rare-cats.jpg',
			imageDescription: 'Domestic shorthair',
			name: 'Peaches',
			sex: 'Female',
			age: 2,
			breed: 'Domestic shorthair',
			story: 'Found in a dumpster and brought in to shelter'
		},
		{
			id: 6,
			imageURL:
				'https://s3.amazonaws.com/filestore.rescuegroups.org/6685/pictures/animals/10459/10459928/54644839_385x385.jpg',
			imageDescription: 'Domestic Medium Hair',
			name: 'Wendy',
			sex: 'Female',
			age: 8,
			breed: 'Domestic Medium Hair',
			story: 'Thrown on the street'
		},
		{
			id: 7,
			imageURL: 'https://www.petguide.com/wp-content/uploads/2015/10/domestic-shorthair-1.jpg',
			imageDescription: 'Domestic Shorthair',
			name: 'Cricket',
			sex: 'Male',
			age: 3,
			breed: 'Domestic Shorthair',
			story: 'Rescue'
		}
	],
	dogs: [
		{
			id: 1,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'A smiling golden-brown golden retreiver listening to music.',
			name: 'Zeus',
			sex: 'Male',
			age: 3,
			breed: 'Golden Retriever',
			story: 'Owner Passed away'
		},
		{
			id: 2,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'Beagle',
			name: 'Ally',
			sex: 'Female',
			age: 3,
			breed: 'Beagle',
			story: 'Found on the street'
		},
		{
			id: 3,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'German Shephard',
			name: 'Banana',
			sex: 'Male',
			age: 2,
			breed: 'German Shephard',
			story: 'Owner Passed away'
		},
		{
			id: 4,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'Great Pyrenees',
			name: 'Chapo',
			sex: 'Male',
			age: 5,
			breed: 'Great Pyrenees',
			story: 'Owner no longer wanted him'
		},
		{
			id: 5,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'Border Collie',
			name: 'Gizmo',
			sex: 'Male',
			age: 1,
			breed: 'Border Collie',
			story: 'Rescue'
		},
		{
			id: 6,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'Siberian Husky',
			name: 'Kodiak',
			sex: 'Female',
			age: 2,
			breed: 'Siberian Husky',
			story: 'Owner Passed away'
		},
		{
			id: 7,
			imageURL: 'http://www.dogster.com/wp-content/uploads/2015/05/Cute%20dog%20listening%20to%20music%201_1.jpg',
			imageDescription: 'Pitbull',
			name: 'Nugget',
			sex: 'Female',
			age: 3,
			breed: 'Pitbull',
			story: 'Found in the street'
		}
	]
};

module.exports = petData;
